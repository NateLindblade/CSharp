...
namespace MyApp
{
    ...
    {
        // courses now points at the shared repository in Library.CMS, so both
        // this console app and a future MAUI app read/write the same data.
        static List<Course> courses = CmsRepository.Courses;
 
        // allStudents now points at the shared repository too, same reasoning as courses.
        static List<Student> allStudents = CmsRepository.AllStudents;
 
        // Note: assignment, submission, and assignment-group Ids are now
        // generated by CmsRepository (see Library.CMS) instead of local fields here.
        ...
        static void SelectCourse()
        {
            ...
            // Group by semester and sort the groups alphabetically by term name
        ...
        static void DeleteCourse()
        {
            ...
            if (int.TryParse(input, out int selectedId))
            {
                // The service only removes the course itself. The students in its
                // roster are references into allStudents, so they aren't touched -
                // they still exist and are still enrolled in their other courses.
        ...
        static void DeleteAssignment(Course course)
        {
            ...
            if (int.TryParse(input, out int selectedId))
            {
                ...
                // Submissions live inside the Assignment object itself, so removing
                // the assignment removes its submissions along with it automatically.
        ...
        static bool UnenrollStudent(Course course, Student currentStudent)
        {
            ...
            else
            {
                ...
                if (int.TryParse(input, out int selectedId))
                {
                    ...
                    // This only removes them from this course's roster.
                    // They still exist in allStudents and stay enrolled in
                    // any other courses they're part of.
